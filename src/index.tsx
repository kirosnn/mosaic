...
if (process.env.MOSAIC_WORKSPACE && !parsed.directory) {
  try {
    process.chdir(process.env.MOSAIC_WORKSPACE);
  } catch {}
}

debugLog(
  `[boot] startup pid=${process.pid} cwd=${process.cwd()} args=${JSON.stringify(args)}`,
);
...
await initializeMcp().catch((e) => {
  debugLog(`MCP init error: ${e}`);
});
    ...
    if (parsed.resumeId) {
      ...
      if (!conversation) {
        ...
        createRoot(renderer).render(
          <App initialMessages={messages} initialTitle={title} />,
        );
      }
    } else {
