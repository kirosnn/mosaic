...
export function ShortcutsModal({ activeTab }: ShortcutsModalProps) {
  const shortcutsGeneral: ShortcutItem[] = [
    { keys: "Ctrl+P / Alt+P", description: "Open/close this shortcuts panel" },
    {
      ...
      description: "Paste from clipboard into the focused input",
    },
  ...
  const shortcutsSetup: ShortcutItem[] = [
    ...
    {
      keys: "Alt+V (or Ctrl+V)",
      description: "Paste API key into the focused input",
    },
  ];
        ...
        >
          ...
          <box marginBottom={1} flexDirection="row" width="100%">
            <box
              paddingLeft={1}
              paddingRight={1}
              backgroundColor={activeTab === 0 ? "#2a2a2a" : "transparent"}
            >
              <text
                fg={activeTab === 0 ? "#D4D4D8" : undefined}
                attributes={
                  activeTab === 0 ? TextAttributes.BOLD : TextAttributes.DIM
                }
              >
                F1 General
              </text>
            ...
            <box
              ...
              paddingLeft={1}
              paddingRight={1}
              backgroundColor={activeTab === 1 ? "#2a2a2a" : "transparent"}
            >
              <text
                fg={activeTab === 1 ? "#D4D4D8" : undefined}
                attributes={
                  activeTab === 1 ? TextAttributes.BOLD : TextAttributes.DIM
                }
              >
                F2 Setup
              </text>
