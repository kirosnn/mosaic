...
export function shouldShowThinkingIndicator({
  isProcessing,
  hasQuestion,
}: ThinkingIndicatorProps): boolean {
  return isProcessing && !hasQuestion;
}

export function getBottomReservedLinesForInputBar(
  props: ThinkingIndicatorProps,
): number {
  const indicatorLines = shouldShowThinkingIndicator(props)
    ? props.nextStep
      ? 3
      : 2
    : 0;
...
export function formatElapsedTime(
  ...
  fromStartTime: boolean = true,
): string {
  if (!ms) return "";
  const elapsed = fromStartTime
...
function createKnightRiderFrames(options: KnightRiderOptions = {}): {
  frames: string[];
  colors: string[];
  defaultColor: string;
} {
...
export function ThinkingIndicator(props: ThinkingIndicatorProps) {
  ...
  if (bd && bd.reasoning > 0)
    parts.push(`reasoning ${bd.reasoning.toLocaleString()}`);
    ...
    <box flexDirection="row" width="100%">
      <box flexDirection="row">
        {sliderFrame.split("").map((char, i) => {
          const idx = calculateColorIndex(
            animTick % slider.frames.length,
            i,
            sliderFrame.length,
            trailOptions,
          );
          const fg =
            idx >= 0 && idx < slider.colors.length
              ? slider.colors[idx]
              : slider.defaultColor;
          const attributes =
            idx === 0 ? TextAttributes.BOLD : TextAttributes.DIM;
          return (
      ...
      {elapsedStr && (
        <text attributes={TextAttributes.DIM}>{` — ${elapsedStr}`}</text>
      )}
      <text attributes={TextAttributes.DIM}>{" — "}</text>
      ...
      {displayedTokens > 0 && (
        <>
          <text attributes={TextAttributes.DIM}>{" — "}</text>
          <text fg={isFlashing ? "#D4D4D8" : "white"}>
            {displayedTokens.toLocaleString()}
          </text>
          <text
            attributes={TextAttributes.DIM}
          >{` tokens${breakdownStr}`}</text>
        </>
      )}
    </box>
  );
      ...
      {props.nextStep ? (
        <box flexDirection="row" width="100%" paddingLeft={2}>
          ...
          {nextLabel.split("").map((char, index) => {
            return (
              <text key={index} fg="#D4D4D8" attributes={TextAttributes.BOLD}>
                {char}
              </text>
            );
          })}
