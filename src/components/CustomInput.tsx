...
function parseSlashInput(
  input: string,
): { prefix: string; query: string; suffix: string } | null {
...
function buildSlashCompletions(input: string): SlashCompletionItem[] {
  ...
  const score = (token: string) => {
    ...
    if (lower.startsWith(query))
      return 0.92 + (query.length / Math.max(1, lower.length)) * 0.06;
...
export function CustomInput({
  ...
  const [hoveredCompletionToken, setHoveredCompletionToken] = useState<
    string | null
  >(null);
  ...
  const lastClipboardPasteRef = useRef<{ at: number; text: string } | null>(
    null,
  );
  const lastClipboardImageRef = useRef<{
    at: number;
    signature: string;
  } | null>(null);
  const lastPasteRequestIdRef = useRef(0);
  const lastPasteUndoRef = useRef<{
    prevValue: string;
    prevCursor: number;
    nextValue: string;
    nextCursor: number;
  ...
  const completions = ENABLE_TUI_COMPLETIONS
    ? buildSlashCompletions(value)
    : [];
  ...
  const addPastedBlock = (pastedText: string) => {
    ...
    const nextValue =
      prevValue.slice(0, insertAt) + normalized + prevValue.slice(deleteUntil);
  ...
  const openExternalEditor = () => {
    const filePath = join(
      tmpdir(),
      `mosaic-input-${Date.now()}-${process.pid}.txt`,
    );
    try {
      ...
      if (process.platform === "win32") {
        ...
        execSync(
          `powershell.exe -NoProfile -Command "Start-Process notepad.exe -ArgumentList '${escaped}' -Wait"`,
          { stdio: "ignore" },
        );
  ...
  const pasteFromClipboard = () => {
    try {
      ...
      let clipboardText = "";
      if (process.platform === "win32") {
        clipboardText = execSync('powershell.exe -command "Get-Clipboard"', {
          encoding: "utf8",
          timeout: 2000,
        });
      } else if (process.platform === "darwin") {
  ...
  useKeyboard((key) => {
    ...
    if (key.name === "a" && (key.ctrl || key.meta)) {
      if (value.length > 0) {
        ...
        const hasSelection =
          ...
          selEnd > selStart;
        const fullSelection =
          hasSelection && selStart === 0 && selEnd === value.length;
        if (fullSelection) {
          setSelectionStart(null);
          setSelectionEnd(null);
    ...
    if (key.name === "tab") {
      ...
      if (total > 0) {
        ...
        if (!selected) return;
        const nextValue = applySlashCompletion(
          valueRef.current,
          selected.token,
        );
        setValue(nextValue);
    ...
    if (key.name === "return") {
      ...
      const {
        ...
        lineStarts,
      } = wrapTextWithCursor(value, cursorPosition, lineWidth);
      ...
      const candidateIndices = getHistoryCandidates(
        historyIndex === -1 ? value : currentInput,
      );
      ...
      if (
        ...
        currentCandidatePosition < candidateIndices.length - 1
      ) {
        const newIndex = candidateIndices[currentCandidatePosition + 1]!;
        setHistoryIndex(newIndex);
        setValue(inputHistory[newIndex]!);
  ...
  if (isEmpty) {
    ...
    return (
      <box flexDirection="column" flexGrow={1} width="100%">
        <box flexDirection="row">
          <text fg="gray" bg="white" attributes={TextAttributes.DIM}>
            {firstChar}
          </text>
          {placeholder.slice(1) && (
  ...
  const { lineStarts, lineLengths, cursorLine, cursorCol } = wrapTextWithCursor(
    value,
    cursorPosition,
    lineWidth,
  );
  ...
  const buildSelectionSegments = (text: string, absStart: number) => {
    ...
    if (selectionRange.start > segStart) {
      parts.push({
        text: text.slice(0, selectionRange.start - segStart),
        selected: false,
      });
    }
        ...
        <box key={lineIndex} flexDirection="row">
          {(() => {
            ...
            if (lineIndex === cursorLine) {
              const safeCursorCol = Math.max(
                0,
                Math.min(line.length, cursorCol),
              );
              ...
              const afterSegs = buildSelectionSegments(
                after,
                lineStart + safeCursorCol + 1,
              );
              return (
                <>
                  {beforeSegs.map((seg, i) => (
                    <text
                      key={`b-${i}`}
                      fg="white"
                      bg={seg.selected ? "#3a3a3a" : undefined}
                    >
                      {seg.text}
                    </text>
                  ))}
          ...
          >
            {completions.map((item, index) => {
              const active =
                ...
                hoveredCompletionToken === item.token;
              return (
                <box
                  ...
                  onMouseOut={() => {
                    setHoveredCompletionToken((prev) =>
                      prev === item.token ? null : prev,
                    );
                  }}
