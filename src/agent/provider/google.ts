...
function extractFunctionCallThoughtSignatures(
  payload: any,
...
function mergeThoughtSignatures(entries: ThoughtSignatureEntry[]): number {
  ...
  if (cachedThoughtSignatures.length > MAX_CACHED_THOUGHT_SIGNATURES) {
    cachedThoughtSignatures = cachedThoughtSignatures.slice(
      -MAX_CACHED_THOUGHT_SIGNATURES,
    );
  }
...
function injectThoughtSignaturesIntoRequest(requestBody: any): {
  injected: number;
  totalModelFunctionCalls: number;
  missing: number;
} {
  ...
  if (modelFunctionCalls.length === 0 || cachedThoughtSignatures.length === 0) {
    ...
    return {
      injected: 0,
      totalModelFunctionCalls: modelFunctionCalls.length,
      missing,
    };
  }
  ...
  for (let i = modelFunctionCalls.length - 1; i >= 0; i--) {
    ...
    let index = findLatestSignatureIndex(
      used,
      (entry) =>
        entry.toolName === call.toolName && entry.argsJson === call.argsJson,
    );
    if (index < 0) {
    ...
    call.part.thoughtSignature =
      cachedThoughtSignatures[index]!.thoughtSignature;
    used.add(index);
    injected++;
  }
  ...
  for (const [toolName, tool] of Object.entries(tools)) {
    ...
    wrapped[toolName] = {
      ...
      execute: async (args: any, options: any) => {
        ...
        if (cache.has(key)) {
          debugLog(
            `[google] tool-cache hit ${toolName} args=${argsJson.slice(0, 100)}`,
          );
...
async function loadCodeAssist(
  ...
  if (!res.ok) {
    ...
    debugLog(
      `[oauth][google] loadCodeAssist failed status=${res.status} body=${text.slice(0, 500)}`,
    );
...
async function onboardUser(
  ...
  if (!res.ok) {
    const text = await res.text();
    debugLog(
      `[oauth][google] onboardUser failed status=${res.status} body=${text.slice(0, 500)}`,
    );
...
async function pollOperation(
  ...
  const res = await fetch(
    `${CODE_ASSIST_ENDPOINT}/${CODE_ASSIST_API_VERSION}/${operationName}`,
    {
      method: "GET",
      headers: codeAssistHeaders(accessToken),
    },
  ...
  if (!res.ok) {
    const text = await res.text();
    debugLog(
      `[oauth][google] getOperation failed status=${res.status} body=${text.slice(0, 500)}`,
    );
...
async function discoverProjectId(accessToken: string): Promise<string> {
  ...
  if (
    cachedProjectId &&
    cachedTierInfo &&
    cachedTierInfo.lastChecked &&
    now - cachedTierInfo.lastChecked < 300000
  ) {
  ...
  debugLog(
    `[oauth][google] session diagnostic: tier=${currentTier} allowed=[${allowedTiers.join(",")}] project=${projectId ?? "none"}`,
  );
  ...
  debugLog(
    `[oauth][google] onboardUser response: done=${lro.done} name=${lro.name ?? "none"}`,
  );
  ...
  if (lro.done && lro.response?.cloudaicompanionProject?.id) {
    ...
    debugLog(
      `[oauth][google] onboarding complete, projectId=${cachedProjectId}`,
    );
    return cachedProjectId;
  }
    ...
    for (let i = 0; i < 24; i++) {
      ...
      if (op.done) {
        if (op.response?.cloudaicompanionProject?.id) {
          ...
          cachedTierInfo.lastChecked = Date.now();
          debugLog(
            `[oauth][google] onboarding complete after polling, projectId=${cachedProjectId}`,
          );
          return cachedProjectId;
        }
        ...
        if (reloaded.cloudaicompanionProject) {
          ...
          cachedTierInfo.lastChecked = Date.now();
          debugLog(
            `[oauth][google] found projectId after reload: ${cachedProjectId}`,
          );
          return cachedProjectId;
        }
        break;
    ...
    async pull(controller) {
      ...
      for (const line of lines) {
        if (line.startsWith("data: ")) {
          ...
          if (!firstEventLogged) {
            debugLog(
              `[oauth][google] first SSE event: ${jsonStr.slice(0, 500)}`,
            );
          ...
          try {
            ...
            if (extracted.length > 0) {
              ...
              debugLog(
                `[oauth][google] captured thoughtSignature for ${extracted.length} function call(s), added=${added}, cache=${cachedThoughtSignatures.length}`,
              );
            }
...
export class GoogleProvider implements Provider {
  async *sendMessage(
    ...
    const refreshOauthIfNeeded = async (
      force = false,
    ): Promise<typeof oauthAuth> => {
      if (!oauthAuth?.refreshToken) return oauthAuth;
      if (
        !force &&
        oauthAuth.expiresAt &&
        Date.now() < oauthAuth.expiresAt - 60000
      )
        return oauthAuth;
      if (force) {
        debugLog(
          `[oauth][google] force refresh requested, invalidating current token`,
        );
      ...
      if (!refreshed) {
        debugLog(
          `[oauth][google] refresh returned null, token may be permanently invalid`,
        );
        return oauthAuth;
      }
      const updated = {
    ...
    const fetchWithOAuth = async (
      ...
      if (model && action) {
        ...
        if (bodyText) {
          try {
            ...
            if (oauthAuth && action === "streamGenerateContent") {
              const injection =
                injectThoughtSignaturesIntoRequest(originalBody);
              if (injection.totalModelFunctionCalls > 0) {
                debugLog(
                  `[oauth][google] thoughtSignature injection injected=${injection.injected} totalModelFunctionCalls=${injection.totalModelFunctionCalls} missing=${injection.missing} cache=${cachedThoughtSignatures.length}`,
                );
              }
            }
        ...
        debugLog(
          `[oauth][google] ${init?.method ?? "POST"} ${url} -> ${targetUrl} model=${model} project=${projectId} token=${maskToken(accessToken)}`,
        );
        ...
        for (let fetchAttempt = 0; ; fetchAttempt++) {
          ...
          if (res.status === 429 && fetchAttempt < maxQuotaRetries) {
            ...
            debugLog(
              `[oauth][google] quota exhausted (attempt ${fetchAttempt + 1}/${maxQuotaRetries}, lightweight=${isLightweight}), waiting ${delay}ms | body=${text.slice(0, 300)}`,
            );
          ...
          if (
            ...
            auth401Retries++;
            debugLog(
              `[oauth][google] auth error ${res.status} (attempt ${auth401Retries}/${MAX_AUTH_RETRIES}) | body=${text.slice(0, 300)}`,
            );
            ...
            if (!refreshed?.accessToken) {
              debugLog(
                `[oauth][google] force refresh failed after ${res.status}, cannot recover`,
              );
            ...
            headers.set("Authorization", `Bearer ${refreshed.accessToken}`);
            try {
              const newProjectId = await discoverProjectId(
              ...
              debugLog(
                `[oauth][google] project re-discovery failed after auth retry: ${projErr instanceof Error ? projErr.message : projErr}`,
              );
            }
            continue;
          }
          ...
          if (!res.ok) {
            ...
            debugLog(
              `[oauth][google] ${targetUrl} status=${res.status} body=${text.slice(0, 500)}`,
            );
          }
      ...
      debugLog(
        `[oauth][google] passthrough ${init?.method ?? "GET"} ${url} token=${maskToken(accessToken)}`,
      );
      const res = await fetch(url, { ...init, headers });
      if (!res.ok) {
        const text = await res.clone().text();
        debugLog(
          `[oauth][google] ${url} status=${res.status} body=${text.slice(0, 500)}`,
        );
      }
      return res;
    };
    ...
    const memoizedTools = buildMemoizedTools(
      config.tools as Record<string, any> | undefined,
    );
    ...
    debugLog(
      `[google] starting stream model=${cleanModel} messagesLen=${messages.length} reasoningRequested=${reasoningEnabled} oauth=${!!oauthAuth}`,
    );
      ...
      yield* runWithRetry(
        async function* () {
          const result = streamText({
            ...
            providerOptions: googleReasoning
            ...
            switch (c.type) {
              case "reasoning":
                const reasoningContent =
                  c.textDelta || c.reasoning || c.content;
                if (reasoningContent) {
                  if (!reasoningCaptured) {
                    debugLog(
                      `[google] readable reasoning content received from provider`,
                    );
              ...
              case "tool-call":
                debugLog(
                  `[google] tool-call ${c.toolName} args=${JSON.stringify(c.args ?? {}).slice(0, 100)}`,
                );
                yield {
                  type: "tool-call-end",
              ...
              case "finish": {
                ...
                if (effectiveFinishReason !== finishReason) {
                  debugLog(
                    "[google] finish reason remapped stop->length due to sanitizer truncation",
                  );
                }
                debugLog(
                  `[google] finish reason=${effectiveFinishReason} promptTokens=${c.usage?.promptTokens ?? "?"} completionTokens=${c.usage?.completionTokens ?? "?"}`,
                );
                yield {
                  type: "finish",
