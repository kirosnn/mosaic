...
import {
  ...
  isSupportedOpenAIOAuthCatalogModelId,
  setOAuthTokenForProvider,
  setOAuthModelsForProvider,
  setFirstRunComplete,
  readConfig,
  getProviderById,
} from "../utils/config";
...
export const OPENAI_CHATGPT_OAUTH_BASE_URL =
...
function buildOAuthPageHtml(
  ...
  const headingColor =
    kind === "success" ? "var(--text-primary)" : "var(--error-color)";
...
export async function startLocalOAuthCallbackServer<T>(options: {
  ...
  callbackPath: string;
  onValidCallback: (
    context: LocalOAuthCallbackContext,
  ) => Promise<OAuthBrowserResult<T>>;
  ...
  const server = http.createServer(async (req, res) => {
    ...
    debugLog(
      `[oauth][${options.providerId}] callback received path=${url.pathname} query=${url.search}`,
    );
    ...
    if (!state || state !== options.expectedState) {
      debugLog(
        `[oauth][${options.providerId}] state rejected expected=${options.expectedState} actual=${state ?? ""}`,
      );
      res.writeHead(400, { "Content-Type": "text/html; charset=utf-8" });
      finalize(null);
      res.end(
    ...
    try {
      ...
      if (outcome.response.type === "redirect") {
        debugLog(
          `[oauth][${options.providerId}] redirect sent to browser location=${outcome.response.location}`,
        );
      ...
      res.writeHead(outcome.response.statusCode, {
      ...
      debugLog(
        `[oauth][${options.providerId}] callback handling failed error=${message}`,
      );
  ...
  const closeServer = () =>
    new Promise<void>((resolve) => {
      ...
      closed = true;
      debugLog(
        `[oauth][${options.providerId}] callback server shutdown requested`,
      );
      server.close(() => {
        debugLog(
          `[oauth][${options.providerId}] callback server shutdown complete`,
        );
        resolve();
      });
    });
  ...
  await new Promise<void>((resolve, reject) => {
    ...
    const onListening = () => {
      ...
      debugLog(
        `[oauth][${options.providerId}] callback server listening address=http://127.0.0.1:${actualPort}${options.callbackPath}`,
      );
      resolve();
    };
...
async function exchangeAuthorizationCode(
  ...
  const result = await exchangeAuthorizationCodeDetailed(
    providerId,
    tokenUrl,
    params,
  );
...
async function exchangeAuthorizationCodeDetailed(
  ...
  { ok: true; tokens: OAuthTokenState } | { ok: false; reason: string }
> {
  ...
  if (!res.ok || data.error) {
    debugLog(
      `[oauth][${providerId}] token exchange failed status=${res.status} error=${data.error ?? ""} desc=${data.error_description ?? ""}`,
    );
    return {
      ok: false,
  ...
  debugLog(
    `[oauth][${providerId}] token exchange ok access=${maskToken(data.access_token)} refresh=${maskToken(data.refresh_token)}`,
  );
  return {
    ok: true,
    tokens: {
...
async function refreshOAuthTokenGeneric(
  ...
  for (let attempt = 0; attempt < REFRESH_MAX_ATTEMPTS; attempt++) {
    debugLog(
      `[oauth][${providerId}] refresh token=${maskToken(fallbackRefreshToken)}${attempt > 0 ? ` (retry ${attempt}/${REFRESH_MAX_ATTEMPTS - 1})` : ""}`,
    );
    ...
    try {
      ...
      debugLog(
        `[oauth][${providerId}] refresh network error: ${err instanceof Error ? err.message : err}`,
      );
    ...
    if (!res.ok || data.error) {
      debugLog(
        `[oauth][${providerId}] refresh failed status=${res.status} error=${data.error ?? ""} desc=${data.error_description ?? ""}`,
      );
      if (
        PERMANENT_OAUTH_ERRORS.includes(data.error ?? "") ||
        (res.status >= 400 &&
          res.status < 500 &&
          res.status !== 408 &&
          res.status !== 429)
    ...
    debugLog(
      `[oauth][${providerId}] refresh ok access=${maskToken(data.access_token)} refresh=${maskToken(data.refresh_token)}`,
    );
    return {
      accessToken: data.access_token,
...
async function syncOpenAICodexModels(): Promise<
...
export async function refreshOpenAIOAuthToken(
  ...
  return refreshOAuthTokenGeneric(
    "openai-oauth",
    OPENAI_TOKEN_URL,
    {
...
async function runOpenAIOAuthFlow(): Promise<boolean> {
  ...
  try {
    ...
    debugLog(
      `[oauth][${providerId}] local callback flow failed error=${error instanceof Error ? error.message : error}`,
    );
  }
...
async function runGoogleOAuthFlow(): Promise<boolean> {
  ...
  if (!hasGoogleOAuthCredentials()) {
    console.error(
      "Google OAuth credentials are missing. Set MOSAIC_GOOGLE_CLIENT_ID and MOSAIC_GOOGLE_CLIENT_SECRET.",
    );
    return false;
  }
  const pkce = generatePKCE();
  ...
  try {
    server = await startLocalOAuthCallbackServer<boolean>({
      ...
      onValidCallback: async ({ code }) => {
        ...
        const exchange = await exchangeAuthorizationCodeDetailed(
          providerId,
          GOOGLE_TOKEN_URL,
          {
            grant_type: "authorization_code",
        ...
        if (!exchange.ok) {
          debugLog(
            `[oauth][${providerId}] token exchange failed reason=${exchange.reason}`,
          );
          return {
            response: {
              type: "html",
    ...
    debugLog(
      `[oauth][${providerId}] callback server failed to start error=${error instanceof Error ? error.message : error}`,
    );
...
export async function runOAuthFlow(providerId: string): Promise<boolean> {
  ...
  console.error(
    `Supported providers: ${getSupportedOAuthProviders().join(", ")}`,
  );
  return false;
}
