...
- A **web UI** for users who prefer a browser-based experience.
...
- **Safe tool execution**: Requires user approval for write/edit operations.
- **Slash commands**: Quick actions like `/init`, `/web`, and `/help`.
...
5. **Web UI** (`src/web/`)
   ...
   - Built with React and served via a lightweight web server.
...
| **AI**            | Vercel AI SDK, OpenAI, Anthropic, Google, Mistral, xAI, Ollama               |
...
- **Approval Policy**: Tools must specify their risk level (e.g., `low`, `medium`, `high`).
...
| `bin/mosaic.cjs`     | CLI entry point.                                                                            |
...
| `web/server.tsx`             | Web server and routing.                                                                     |
| `utils/`                     | Shared utilities and helpers.                                                               |
| `utils/config.ts`            | Configuration management (e.g., AI providers, API keys).                                    |
| `utils/commands/`            | Slash command implementations (e.g., `/init`, `/web`).                                      |

### Configuration
| File/Directory               | Purpose                                                                                     |
|--------------------------------|---------------------------------------------------------------------------------------------|
| `~/.mosaic/mosaic.jsonc`      | Global configuration (providers, API keys, settings).                                       |
...
### Running the TUI
- Start the TUI: `bun run dev` or `npm run dev`.
- Use slash commands (e.g., `/help`, `/web`).
- Interact with the agent via chat.

### Running the Web UI
1. Start the web server: `mosaic web`.
...
2. Write unit tests in `src/agent/tools/__tests__/`.
3. Run tests: `bun test` or `npm test`.

### Debugging
- Use `console.debug` for debug logs (controlled via `DEBUG` environment variable).
...
- **Tool Safety**: Never execute write/edit operations without user approval.
